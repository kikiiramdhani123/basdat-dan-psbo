...
class Adm extends CI_Controller {
	...
	public function m_siswa() {
		...
		if ($uri3 == "det") {
			$a = $this->db->query("SELECT * FROM murid WHERE id = '$uri4'")->row();
			j($a);
			exit();
		} else if ($uri3 == "simpan") {
			$ket 	= "";
			if ($p->id != 0) {
				$this->db->query("UPDATE murid SET nama = '".bersih($p,"nama")."', nim = '".bersih($p,"nim")."', jurusan = '".bersih($p,"jurusan")."', alamat = '".bersih($p, "alamat")."', tempat_lahir = '".bersih($p, "tempat_lahir")."', jenis_kelamin = '".bersih($p,"jenis_kelamin")."', tanggal_lahir = '".bersih($p, "tanggal_lahir")."' WHERE id = '".bersih($p,"id")."'");
				$ket = "edit";
			} else {
				$ket = "tambah";
				$this->db->query("INSERT INTO murid VALUES (null, '".bersih($p,"nama")."', '".bersih($p,"nim")."', '".bersih($p,"jurusan")."', '".bersih($p,"alamat")."', '".bersih($p,"tempat_lahir")."', '".bersih($p,"tanggal_lahir")."', '".bersih($p,"jenis_kelamin")."')");
			}
			...
			$this->db->query("DELETE FROM murid WHERE id = '".$uri4."'");
			$this->db->query("DELETE FROM admin WHERE level = 'siswa' AND kon_id = '".$uri4."'");			
			$ret_arr['status'] 	= "ok";
			$ret_arr['caption']	= "hapus sukses";
			j($ret_arr);
			exit();
		} else if ($uri3 == "user") {
			$det_user = $this->db->query("SELECT id, nim FROM murid WHERE id = '$uri4'")->row();

			if (!empty($det_user)) {
				$q_cek_username = $this->db->query("SELECT id FROM admin WHERE username = '".$det_user->nim."' AND level = 'siswa'")->num_rows();

				if ($q_cek_username < 1) {

					$this->db->query("INSERT INTO admin VALUES (null, '".$det_user->nim."', md5('".$det_user->nim."'), 'siswa', '".$det_user->id."')");
					$ret_arr['status'] 	= "ok";
			...
			$this->db->query("UPDATE admin SET password = md5('".$det_user->nim."') WHERE level = 'siswa' AND kon_id = '".$det_user->id."'");
			...
			$matkul = $this->db->query("SELECT modul.*,
										(SELECT COUNT(id) FROM tr_siswa_mapel WHERE id_siswa = ".$uri4." AND id_mapel = modul.id) AS ok
										FROM modul
										")->result();
			$ret_arr['status'] = "ok";
			$ret_arr['data'] = $matkul;
			...
			$ket 	= "";
			//echo var_dump($p);
			$ambil_matkul = $this->db->query("SELECT id FROM modul ORDER BY id ASC")->result();
	        ...
	        $d_total_row = $this->db->query("SELECT id FROM murid a WHERE a.nama LIKE '%".$search['value']."%'")->num_rows();
	    
	        $q_datanya = $this->db->query("SELECT a.*,
											(SELECT COUNT(id) FROM admin WHERE level = 'siswa' AND kon_id = a.id) AS ada
											FROM murid a
	                                        WHERE a.nama LIKE '%".$search['value']."%' ORDER BY a.id DESC")->result_array();
	        $data = array();
	        $no = ($start+1);
	...
	public function m_guru() {
		...
		$jeson = array();
		/*
		$a['data'] = $this->db->query("SELECT guru.*,
									(SELECT COUNT(id) FROM admin WHERE level = 'guru' AND kon_id = guru.id) AS ada
									FROM guru")->result();
		*/
		...
		if ($uri3 == "det") {
			...
			$ket 	= "";
			if ($p->id != 0) {
				$this->db->query("UPDATE guru SET nama = '".bersih($p,"nama")."', nip = '".bersih($p,"nip")."', alamat = '".bersih($p,"alamat")."', tempat_lahir = '".bersih($p,"tempat_lahir")."', tanggal_lahir = '".bersih($p,"tanggal_lahir")."', jenis_kelamin = '".bersih($p,"jenis_kelamin")."', lulusan = '".bersih($p,"lulusan")."' WHERE id = '".bersih($p,"id")."'");
				$ket = "edit";
			} else {
				$ket = "tambah";
				$this->db->query("INSERT INTO guru VALUES (null, '".bersih($p,"nip")."', '".bersih($p,"nama")."', '".bersih($p,"alamat")."', '".bersih($p,"tempat_lahir")."', '".bersih($p,"tanggal_lahir")."', '".bersih($p,"jenis_kelamin")."', '".bersih($p,"lulusan")."')");
			}
			...
			$this->db->query("DELETE FROM admin WHERE level = 'guru' AND kon_id = '".$uri4."'");
			$ret_arr['status'] 	= "ok";
			$ret_arr['caption']	= "hapus sukses";
			j($ret_arr);
			exit();
			...
			if (!empty($det_user)) {
				$q_cek_username = $this->db->query("SELECT id FROM admin WHERE username = '".$det_user->nip."' AND level = 'guru'")->num_rows();

				if ($q_cek_username < 1) {

					$this->db->query("INSERT INTO admin VALUES (null, '".$det_user->nip."', md5('".$det_user->nip."'), 'guru', '".$det_user->id."')");
					$ret_arr['status'] 	= "ok";
					$ret_arr['caption']	= "tambah user sukses";
					j($ret_arr);
				} else {
			...
			$this->db->query("UPDATE admin SET password = md5('".$det_user->nip."') WHERE level = 'guru' AND kon_id = '".$det_user->id."'");
			...
			$matkul = $this->db->query("SELECT modul.*,
										(SELECT COUNT(id) FROM guru_modul WHERE id_guru = ".$uri4." AND id_mapel = modul.id) AS ok
										FROM modul
										")->result();
			$ret_arr['status'] = "ok";
			$ret_arr['data'] = $matkul;
			...
			if (!empty($ambil_matkul)) {
				foreach ($ambil_matkul as $a) {
					...
					if (!empty($p->$p_sub)) {
						...
						$cek_sudah_ada = $this->db->query("SELECT id FROM guru_modul WHERE  id_guru = '".$p->id_mhs."' AND id_mapel = '".$a->id."'")->num_rows();
						
						if ($cek_sudah_ada < 1) {
							$this->db->query("INSERT INTO guru_modul VALUES (null, '".$p->id_mhs."', '".$a->id."')");
						} else {
							$this->db->query("UPDATE guru_modul SET id_mapel = '".$p->$p_sub."' WHERE id_guru = '".$p->id_mhs."' AND id_mapel = '".$a->id."'");
						}
					} else {
						//echo "0<br>";
						$this->db->query("DELETE FROM guru_modul WHERE id_guru = '".$p->id_mhs."' AND id_mapel = '".$a->id."'");
					}
				}
			}
			$ret_arr['status'] 	= "ok";
	        ...
	        $q_datanya = $this->db->query("SELECT a.*,
											(SELECT COUNT(id) FROM admin WHERE level = 'guru' AND kon_id = a.id) AS ada
											FROM guru a
	...
	public function m_mapel() {
		...
		//return as json
		$jeson = array();
		$a['data'] = $this->db->query("SELECT modul.* FROM modul")->result();
		if ($uri3 == "det") {
			$a = $this->db->query("SELECT * FROM modul WHERE id = '$uri4'")->row();
			j($a);
			exit();
		} else if ($uri3 == "simpan") {
			$ket 	= "";
			if ($p->id != 0) {
				$this->db->query("UPDATE modul SET nama = '".bersih($p,"nama")."'
								WHERE id = '".bersih($p,"id")."'");
				$ket = "edit";
			} else {
				$ket = "tambah";
				$this->db->query("INSERT INTO modul VALUES (null, '".bersih($p,"nama")."')");
			}
			...
			$this->db->query("DELETE FROM modul WHERE id = '".$uri4."'");
			$ret_arr['status'] 	= "ok";
			$ret_arr['caption']	= "hapus sukses";
			j($ret_arr);
			exit();
	        ...
	        $d_total_row = $this->db->query("SELECT id FROM modul a WHERE a.nama LIKE '%".$search['value']."%'")->num_rows();
	...
	public function m_soal() {
		...
		if ($a['sess_level'] == "guru") {
			...
			$a['p_mapel'] = obj_to_array($this->db->query("SELECT 
											b.id, b.nama
											FROM guru_modul a
											INNER JOIN modul b ON a.id_mapel = b.id
			...
			$a['p_mapel'] = obj_to_array($this->db->query("SELECT 
											b.id, b.nama
											FROM guru_modul a
											INNER JOIN modul b ON a.id_mapel = b.id")->result(), "id,nama");
		}

		if ($uri3 == "det") {
			$a = $this->db->query("SELECT * FROM soal WHERE id = '$uri4' ORDER BY id DESC")->row();
			j($a);
			exit();
		} else if ($uri3 == "import") {
			$a['p']	= "pages/f_soal_import";
		} else if ($uri3 == "hapus_gambar") {
			$nama_gambar = $this->db->query("SELECT file FROM soal WHERE id = '".$uri5."'")->row();
			$this->db->query("UPDATE soal SET file = '', tipe_file = '' WHERE id = '".$uri5."'");
			@unlink("./upload/gambar_soal/".$nama_gambar->file);
			redirect('adm/m_soal/pilih_mapel/'.$uri4);
		} else if ($uri3 == "pilih_mapel") {
			if ($a['sess_level'] == "guru") {
				$a['data'] = $this->db->query("SELECT soal.*, guru.nama AS nama_guru FROM soal INNER JOIN guru ON soal.id_guru = guru.id WHERE soal.id_guru = '".$a['sess_konid']."' AND soal.id_mapel = '$uri4' ORDER BY id DESC")->result();
			} else {
				$a['data'] = $this->db->query("SELECT soal.*, guru.nama AS nama_guru FROM soal INNER JOIN guru ON soal.id_guru = guru.id WHERE soal.id_mapel = '$uri4' ORDER BY id DESC")->result();
			}
			...
			if ($__mode == "edit") {
				...
				$this->db->update("soal", $pdata);
				$__id_soal = $p['id'];
			} else {
				$this->db->insert("soal", $pdata);
				$get_id_akhir = $this->db->query("SELECT MAX(id) maks FROM soal LIMIT 1")->row_array();
			...
			//ambil data awal
			$get_opsi_awal = $this->db->query("SELECT opsi_a, opsi_b, opsi_c, opsi_d, opsi_e FROM soal WHERE id = '".$__id_soal."'")->row_array();
			...
			$this->db->update("soal", $data_simpan);
			...
			$a['p_mapel'] = obj_to_array($this->db->query("SELECT b.id, b.nama FROM guru_modul a INNER JOIN modul b ON a.id_mapel = b.id $id_guru")->result(),"id,nama");

			if ($uri4 == 0) {
				$a['d'] = array("mode"=>"add","id"=>"0","id_guru"=>$id_guru,"id_mapel"=>"","bobot"=>"1","file"=>"","soal"=>"","opsi_a"=>"#####","opsi_b"=>"#####","opsi_c"=>"#####","opsi_d"=>"#####","opsi_e"=>"#####","jawaban"=>"","tgl_input"=>"");
			} else {
				$a['d'] = $this->db->query("SELECT soal.*, 'edit' AS mode FROM soal WHERE id = '$uri4'")->row_array();
			...
			$nama_gambar = $this->db->query("SELECT id_mapel, file, opsi_a, opsi_b, opsi_c, opsi_d, opsi_e FROM soal WHERE id = '".$uri4."'")->row();
			...
			$this->db->query("DELETE FROM soal WHERE id = '".$uri4."'");
			@unlink("./upload/gambar_soal/".$nama_gambar->file);
			...
			if ($a['sess_level'] == "admin") {
				$data = $this->db->query("SELECT * FROM soal")->result();
			} else {
				$data = $this->db->query("SELECT * FROM soal WHERE id_guru = '".$a['sess_konid']."'")->result();
			}

			$mapel = $this->db->query("SELECT nama FROM modul WHERE id = '".$uri4."'")->row();
			if (!empty($data)) {
		        ...
		        $d_total_row = $this->db->query("SELECT a.*
												...
												INNER JOIN modul c ON a.id_mapel = c.id
	...
	public function m_ujian() {
		...
		$a['p_mapel'] = obj_to_array($this->db->query("SELECT * FROM modul WHERE id IN (SELECT id_mapel FROM guru_modul WHERE id_guru = '".$a['sess_konid']."')")->result(), "id,nama");
		
		if ($uri3 == "det") {
			$are = array();

			$a = $this->db->query("SELECT * FROM guru_tes WHERE id = '$uri4'")->row();
			...
			if ($p->id != 0) {
				$this->db->query("UPDATE guru_tes SET id_mapel = '".bersih($p,"mapel")."', 
				...
				$this->db->query("INSERT INTO guru_tes VALUES (null, '".$a['sess_konid']."', '".bersih($p,"mapel")."',
			...
			$this->db->query("DELETE FROM guru_tes WHERE id = '".$uri4."'");
			$ret_arr['status'] 	= "ok";
			$ret_arr['caption']	= "hapus sukses";
			j($ret_arr);
			exit();
		} else if ($uri3 == "jumlah_soal") {
			$ambil_data = $this->db->query("SELECT id FROM soal WHERE id_mapel = '$uri4' AND id_guru = '".$a['sess_konid']."'")->num_rows();
			...
			$this->db->query("UPDATE guru_tes SET token = '$token' WHERE id = '$uri4'");

			$ret_arr['status'] = "ok";
			j($ret_arr);
			exit();
		} else {
			$a['p']	= "pages/m_guru_tes";
		}
		$this->load->view('main', $a);
	}
	public function h_ujian() {
		...
		$a['p_mapel'] = obj_to_array($this->db->query("SELECT * FROM modul")->result(), "id,nama");
		
		if ($uri3 == "det") {
			$a['detil_tes'] = $this->db->query("SELECT modul.nama AS namaMapel, guru.nama AS nama_guru, 
												guru_tes.* 
												FROM guru_tes 
												INNER JOIN modul ON guru_tes.id_mapel = modul.id
												INNER JOIN guru ON guru_tes.id_guru = guru.id
												WHERE guru_tes.id = '$uri4'")->row();
			$a['statistik'] = $this->db->query("SELECT MAX(nilai) AS max_, MIN(nilai) AS min_, AVG(nilai) AS avg_ 
											FROM ujian
											WHERE ujian.id_tes = '$uri4'")->row();

			//$a['hasil'] = $this->db->query("")->result();
			$a['p'] = "pages/m_guru_tes_hasil_detil";
			//echo $this->db->last_query();
	        ...
	        $d_total_row = $this->db->query("
	        	SELECT a.id
				FROM ujian a
				INNER JOIN murid b ON a.id_user = b.id
				WHERE a.id_tes = '$uri4' 
	        ...
	        $q_datanya = $this->db->query("
				...
				INNER JOIN murid b ON a.id_user = b.id
				WHERE a.id_tes = '$uri4' 
				AND b.nama LIKE '%".$search['value']."%' ORDER BY a.id DESC")->result_array(); // LIMIT ".$start.", ".$length."
			...
			$this->db->query("DELETE FROM ujian WHERE id = '$uri4'");
	        ...
	        $d_total_row = $this->db->query("SELECT a.id FROM guru_tes a
	        	INNER JOIN modul b ON a.id_mapel = b.id 
	        ...
	        $q_datanya = $this->db->query("SELECT a.*, b.nama AS modul, c.nama AS nama_guru FROM guru_tes a
	        	INNER JOIN modul b ON a.id_mapel = b.id 
	        	INNER JOIN guru c ON a.id_guru = c.id
	            WHERE (a.nama_ujian LIKE '%".$search['value']."%' OR b.nama LIKE '%".$search['value']."%' OR c.nama LIKE '%".$search['value']."%') ".$wh_1." ORDER BY a.id DESC")->result_array();
	...
	public function hasil_ujian_cetak() {
		...
		$a['detil_tes'] = $this->db->query("SELECT modul.nama AS namaMapel, guru.nama AS nama_guru, 
												...
												INNER JOIN modul ON guru_tes.id_mapel = modul.id
												INNER JOIN guru ON guru_tes.id_guru = guru.id
												WHERE guru_tes.id = '$uri3'")->row();
		
		$a['statistik'] = $this->db->query("SELECT MAX(nilai) AS max_, MIN(nilai) AS min_, AVG(nilai) AS avg_ 
										FROM ujian
										WHERE ujian.id_tes = '$uri3'")->row();
		$a['hasil'] = $this->db->query("SELECT siswa.nama, ujian.nilai, ujian.jml_benar, ujian.nilai_bobot
										FROM ujian
										INNER JOIN siswa ON ujian.id_user = siswa.id
										WHERE ujian.id_tes = '$uri3'")->result();
	...
	public function ikuti_ujian() {
		...
		$a['data'] = $this->db->query("SELECT 
									...
									INNER JOIN modul b ON a.id_mapel = b.id
									INNER JOIN guru c ON a.id_guru = c.id
									LEFT JOIN ujian d ON CONCAT('".$a['sess_konid']."',a.id) = CONCAT(d.id_user,d.id_tes)
									ORDER BY a.id ASC")->result();
		//echo $this->db->last_query();
		$a['p']	= "pages/m_list_ujian_siswa";
		$this->load->view('main', $a);
	}
	public function ikut_ujian() {
		...
		//var post from json
		$p = json_decode(file_get_contents('php://input'));
		$a['detil_user'] = $this->db->query("SELECT * FROM modul WHERE id = '".$a['sess_konid']."'")->row();
		if ($uri3 == "simpan_satu") {
			$p			= json_decode(file_get_contents('php://input'));
			...
			$this->db->query("UPDATE ujian SET list_jawaban = '".$update_."' WHERE id_tes = '$uri4' AND id_user = '".$a['sess_konid']."'");
			//echo $this->db->last_query();

			$q_ret_urn 	= $this->db->query("SELECT list_jawaban FROM ujian WHERE id_tes = '$uri4' AND id_user = '".$a['sess_konid']."'");
			...
			for ($i = 1; $i < $p->jml_soal; $i++) {
				...
				$_tidsoal 	= "id_soal_".$i;
				$jawaban_ 	= empty($p->$_tjawab) ? "" : $p->$_tjawab;
				$cek_jwb 	= $this->db->query("SELECT bobot, jawaban FROM soal WHERE id = '".$p->$_tidsoal."'")->row();
				//untuknilai bobot
				...
				if ($cek_jwb->jawaban == $jawaban_) {
					...
					$q_update_jwb = "UPDATE soal SET jml_benar = jml_benar + 1 WHERE id = '".$p->$_tidsoal."'";
				} else {
					//jika jawaban salah
					$array_nilai[$bobotnya] = empty($array_nilai[$bobotnya]) ? 0 : $array_nilai[$bobotnya] + 0;
					$q_update_jwb = "UPDATE soal SET jml_salah = jml_salah + 1 WHERE id = '".$p->$_tidsoal."'";
				}
			...
			exit;
			*/
			$this->db->query("UPDATE ujian SET jml_benar = ".$jumlah_benar.", nilai_bobot = ".$nilai_bobot.", nilai = '".$nilai."', list_jawaban = '".$update_."', status = 'N' WHERE id_tes = '$uri4' AND id_user = '".$a['sess_konid']."'");
			$a['status'] = "ok";
			j($a);
			exit;		
		} else if ($uri3 == "token") {
			$a['du'] = $this->db->query("SELECT a.id, a.tgl_mulai, a.terlambat, 
										...
										b.nama nmguru, c.nama nmmapel FROM guru_tes a 
										INNER JOIN guru b ON a.id_guru = b.id
										INNER JOIN modul c ON a.id_mapel = c.id 
										WHERE a.id = '$uri4'")->row_array();
			$a['dp'] = $this->db->query("SELEcT * FROM murid WHERE id = '".$a['sess_konid']."'")->row_array();
			...
			$cek_sdh_selesai= $this->db->query("SELECT id FROM ujian WHERE id_tes = '$uri4' AND id_user = '".$a['sess_konid']."' AND status = 'N'")->num_rows();
			
			//sekalian validasi waktu sudah berlalu...
			if ($cek_sdh_selesai < 1) {
				//ini jika ujian belum tercatat, belum ikut
				//ambil detil soal
				$cek_detil_tes = $this->db->query("SELECT * FROM guru_tes WHERE id = '$uri4'")->row();
				$q_cek_sdh_ujian= $this->db->query("SELECT id FROM ujian WHERE id_tes = '$uri4' AND id_user = '".$a['sess_konid']."'");
				...
				if ($cek_sdh_ujian < 1)	{		
					$soal_urut_ok = array();
					$q_soal			= $this->db->query("SELECT id, file, tipe_file, soal, opsi_a, opsi_b, opsi_c, opsi_d, opsi_e, '' AS jawaban FROM soal WHERE id_mapel = '".$cek_detil_tes->id_mapel."' AND id_guru = '".$cek_detil_tes->id_guru."' ".$acakan." LIMIT ".$cek_detil_tes->jumlah_soal)->result();
					$i = 0;
					foreach ($q_soal as $s) {
					...
					$this->db->query("INSERT INTO ujian VALUES (null, '$uri4', '".$a['sess_konid']."', '$list_id_soal', '$list_jw_soal', 0, 0, 0, '$time_mulai', ADDTIME('$time_mulai', '$waktu_selesai'), 'Y')");
					
					$detil_tes = $this->db->query("SELECT * FROM ujian WHERE id_tes = '$uri4' AND id_user = '".$a['sess_konid']."'")->row();

					$soal_urut_ok= $soal_urut_ok;
				} else {
					$q_ambil_soal 	= $this->db->query("SELECT * FROM ujian WHERE id_tes = '$uri4' AND id_user = '".$a['sess_konid']."'")->row();
					...
					for ($i = 0; $i < sizeof($urut_soal); $i++) {
						...
						$ambil_soal = $this->db->query("SELECT *, $pc_urut_soal1 AS jawaban FROM soal WHERE id = '".$pc_urut_soal[0]."'")->row();
				...
				$this->load->view('pages/v_ujian', $a);
			} else {
	...
	public function jvs() {
		...
		$data_soal 		= $this->db->query("SELECT id, gambar, soal, opsi_a, opsi_b, opsi_c, opsi_d, opsi_e FROM soal ORDER BY RAND()")->result();
	...
	public function rubah_password() {
		...
		if ($uri3 == "simpan") {
			...
			$p2_md5 = md5($p->p2);
			$p3_md5 = md5($p->p3);
			$cek_pass_lama = $this->db->query("SELECT password FROM admin WHERE id = '".$a['sess_admin_id']."'")->row();
			if ($cek_pass_lama->password != $p1_md5) {
				...
				$this->db->query("UPDATE admin SET password = '".$p3_md5."' WHERE id = '".$a['sess_admin_id']."'");
				$ret['status'] = "ok";
			...
			$data = $this->db->query("SELECT id, kon_id, level, username FROM admin WHERE id = '".$a['sess_admin_id']."'")->row();
			j($data);
			exit;
		}
	}
	public function sudah_selesai_ujian() {
		...
		$q_nilai = $this->db->query("SELECT nilai, tgl_selesai FROM ujian WHERE id_tes = $uri3 AND id_user = '".$a['sess_konid']."' AND status = 'N'")->row();
		if (empty($q_nilai)) {
			redirect('adm/ikut_ujian/_/'.$uri3);
		} else {
			$a['p'] = "pages/v_selesai_ujian";
	...
	public function act_login() {
		...
		$q_data		= $this->db->query("SELECT * FROM admin WHERE username = '".$username."' AND password = '$password2'");
		...
		if ($j_data === 1) {
			...
			if ($a_data->level == "siswa") {
				$det_user = $this->db->query("SELECT nama FROM murid WHERE id = '".$a_data->kon_id."'")->row();
				if (!empty($det_user)) {
